/*
Example directly copied from https://learnopengl.com/code_viewer_gh.php?code=src/1.getting_started/4.2.textures_combined/textures_combined.cpp
Adapted examples are 05_textures1.cpp, 05_textures2.cpp, 05_textures3.cpp
...
int main()
{
    ...
    unsigned char *data = stbi_load(text1_path.c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    data = stbi_load(text2_path.c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    // -------------------------------------------------------------------------------------------
    ourShader.activate_shader(); // don't forget to activate/use the shader before setting uniforms!
