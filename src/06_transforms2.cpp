...
int main()
{
    ...
    // SHADER CONFIG
    std::string vertex_shader = (binary_path / "../shaders/06_transforms2.vs").string();
    std::string fragment_shader = (binary_path / "../shaders/06_transforms2.fs").string();
    ...
    int transform_uniform_location = glGetUniformLocation(ourShader.ID, "transform");
    ...
    glUniformMatrix4fv(transform_uniform_location, 1, GL_FALSE, glm::value_ptr(trans));
    ...
    while (!glfwWindowShouldClose(my_window->window_ptr))
    {
        ...
        // Transformations 1
        trans = glm::mat4(1.0f);
        trans = glm::rotate(trans, ((float)glfwGetTime()) * 0.6f, glm::vec3(0.0f, 0.0f, 1.0f));
        trans = glm::translate(trans, glm::vec3(0.5f, -0.5f, 0.0f));
        trans = glm::scale(trans, glm::vec3(0.5, 0.5, 0.5));
        glUniformMatrix4fv(transform_uniform_location, 1, GL_FALSE, glm::value_ptr(trans));
        ...
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans, glm::vec3(-0.5f, 0.5f, 0.0f));
        trans = glm::rotate(trans, -0.8f * ((float)glfwGetTime()), glm::vec3(0.0f, 0.0f, 1.0f));
        trans = glm::scale(trans, glm::vec3(0.5, 0.5, 0.5));
        glUniformMatrix4fv(transform_uniform_location, 1, GL_FALSE, glm::value_ptr(trans));
